...
# from keras.layers import LSTM, Dense, Conv1D, LeakyReLU, BatchNormalization, Reshape, Input
...
#             try:
#                 preprocess, label, test_df = Preprocess.preprocessData(full_data)
#                 print(f"Preprocess shape: {preprocess.shape}")
...
#             # Generate Feature Importance (CHART 2)
#             feature_importance_fig = px.bar(feature_score, x='score', y='feature', orientation='h',
...
#             # Train-test split
#             test_size = min(0.3, max(0.1, 30 / len(features_array)))
#             try:
#                 X_train, X_test, y_train, y_test = train_test_split(
#                     features_array, target, test_size=test_size, shuffle=False, random_state=42
#                 )
#             except:
...
#             # Build GAN model (Streamlit architecture)
#             np.random.seed(42)
            
#             # Generator (LSTM)
#             generator = Sequential([
#                 LSTM(200, activation='relu', input_shape=(X_train_lstm.shape[1], X_train_lstm.shape[2])),
...
#             discriminator.add(Conv1D(32, kernel_size=3, strides=2, input_shape=input_shape, padding='same'))
#             discriminator.add(LeakyReLU(0.01))
#             discriminator.add(Dense(1, activation='sigmoid'))
#             discriminator.add(Conv1D(64, kernel_size=3, strides=2, padding='same'))
#             discriminator.add(LeakyReLU(0.01))
#             discriminator.add(BatchNormalization())
#             discriminator.add(Conv1D(128, kernel_size=3, strides=2, padding='same'))
#             discriminator.add(LeakyReLU(0.01))
#             discriminator.add(BatchNormalization())
#             discriminator.add(Dense(220))
#             discriminator.add(LeakyReLU(0.01))
#             discriminator.add(Dense(220))
#             discriminator.add(LeakyReLU(0.01))
#             discriminator.add(Dense(3, activation='softmax'))
...
#             gan.compile(loss='categorical_crossentropy', optimizer=Adam(learning_rate=0.001))

#             # Train model (Streamlit parameters)
#             batch_size = 64
#             epochs = 100
#             print(f"Training with batch_size={batch_size}, epochs={epochs}")
            
#             try:
#                 generator.fit(X_train_lstm, y_train, epochs=epochs, batch_size=batch_size, verbose=0)
#             except Exception as e:
#                 print(f"Training error: {e}")
#                 generator.fit(X_train_lstm, y_train, epochs=50, batch_size=min(batch_size, len(X_train_lstm)), verbose=0)
...
#             # Generate Final Predictions (CHART 3)
...
#             # Calculate metrics
#             cm = confusion_matrix(actual_target, prediction_labels)
#             accuracy = accuracy_score(actual_target, prediction_labels)
#             recall = recall_score(actual_target, prediction_labels, average='weighted', zero_division=0)
#             precision = precision_score(actual_target, prediction_labels, average='weighted', zero_division=0)
#             f1 = f1_score(actual_target, prediction_labels, average='weighted', zero_division=0)

#             # Generate Confusion Matrix (CHART 4)
#             fig_cm = ff.create_annotated_heatmap(z=cm, x=['Buy', 'Sell', 'Hold'], y=['Buy', 'Sell', 'Hold'],
#                                               colorscale='RdYlBu_r', showscale=True)
#             fig_cm.update_layout(title='Confusion Matrix - Model Performance',
...
#                     features_array, target, target_labels, test_size=test_size, 
...
#                 X_train, X_test = features_array[:split_idx], features_array[split_idx:]
#                 y_train, y_test = target[:split_idx], target[split_idx:]
#                 y_train_labels, y_test_labels = target_labels[:split_idx], target_labels[split_idx:]
...
#             if len(X_train) < 10:
#                 raise ValueError(f"Insufficient training data. Need at least 10 samples, got {len(X_train)}")
...
#                 print(f"Test actual distribution: {np.bincount(y_test_labels)}")
#             except Exception as prediction_error:
#                 print(f"Prediction error: {prediction_error}")
#                 # Create dummy predictions if prediction fails
#                 prediction_labels = np.random.choice([0, 1, 2], size=len(y_test_labels))
...
#                 if len(y_test_labels) != len(prediction_labels):
#                     min_len = min(len(y_test_labels), len(prediction_labels))
#                     y_test_labels_aligned = y_test_labels[:min_len]
#                     prediction_labels_aligned = prediction_labels[:min_len]
#                     print(f"Aligned arrays to length: {min_len}")
#                 else:
#                     y_test_labels_aligned = y_test_labels
#                     prediction_labels_aligned = prediction_labels

#                 cm = confusion_matrix(y_test_labels_aligned, prediction_labels_aligned)
#                 accuracy = accuracy_score(y_test_labels_aligned, prediction_labels_aligned)
#                 recall = recall_score(y_test_labels_aligned, prediction_labels_aligned, average='weighted', zero_division=0)
#                 precision = precision_score(y_test_labels_aligned, prediction_labels_aligned, average='weighted', zero_division=0)
#                 f1 = f1_score(y_test_labels_aligned, prediction_labels_aligned, average='weighted', zero_division=0)
...
#     def run_model(self, full_data: pd.DataFrame, selected_df: pd.DataFrame = None):
#         try:
#             print(f"Full data shape: {full_data.shape}")
#             print(f"Full data columns: {full_data.columns.tolist()}")
...
#             data_hash = self.cache.generate_data_hash(full_data)
#             print(f"Data hash: {data_hash}")
            
#             # Try to load from cache first
#             cached_result = self.cache.load_cache(data_hash)
...
#                 try:
#                     self.cache.save_cache(data_hash, generator, scaler, results, metrics)
...
# # result = predictor.run_model(full_data, selected_df)
